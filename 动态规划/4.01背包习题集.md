...
这里相比01背包，每件物品多了一个q的限制条件，满足条件才能装入背包。可以这样理解，01背包的每件物品限制q = 0，其只考虑该物品要不要放入，但是当q不为零的时候，我们还要考虑该物品什么时候放入，这里产生了顺序的限制。此外还得考虑一点，q这个限制什么时候有效，如果q <= p，就说明这个限制并不会产生影响，因为价格比限制高啊。所以重点在于q > p的时候，q的值会影响物品的放入顺序，为了保证v最大，我们需要优先放入某件物品，但是这个排序的指标是什么呢？慢慢想想，可以试一试用p和q的差值，
